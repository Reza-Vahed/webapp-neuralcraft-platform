...
- **`output: "standalone"`** (seit Phase 10, `next.config.ts`): Next.js traced pro Seite nur die tatsächlich benötigten Dateien in `.next/standalone` (inkl. eines minimalen `server.js`) — Grundlage für ein schlankes Docker-Image ohne `node_modules`, siehe DEPLOYMENT.md. Verifiziert: next-intls `messages/*.json` werden vom Bundler direkt in den kompilierten Server-Code einkompiliert (dynamischer `import()` mit Template-Literal in `i18n/request.ts` wird als Context-Modul aufgelöst) — sie müssen **nicht** separat in den Standalone-Output kopiert werden. `public/` und `.next/static/` dagegen schon (Next.js' eigene Konvention, im `Dockerfile` umgesetzt).
- **`GET /api/health`** (seit Phase 10, `app/api/health/route.ts`): `force-dynamic`-Route-Handler, liefert `status`/`version` (aus `package.json`)/`buildTime` (vom `Dockerfile` als Build-Arg gesetzt)/`uptime` (`process.uptime()`) als JSON. Grundlage für Docker-`HEALTHCHECK` und späteres externes Uptime-Monitoring, siehe DEPLOYMENT.md.
...
- **Environment-Variablen**: alle Secrets (`RESEND_API_KEY`, `TURNSTILE_SECRET_KEY`, …) werden ausschließlich in Dateien mit `import "server-only"` oder `"use server"` gelesen — nie in einer Client-Komponente, nie mit `NEXT_PUBLIC_`-Präfix. `.env.example`/`.env.production.example` dokumentieren alle Variablen ohne echte Werte; `.env*` ist in `.gitignore`.
- **`lib/site.ts` nutzt `||`, nicht `??`** (seit Phase 10): ein leerer, aber _definierter_ `NEXT_PUBLIC_SITE_URL`-Wert (z. B. von einem Docker-Build-Arg ohne Angabe) ist genauso „unset" wie `undefined` — eine Seiten-URL ist nie legitim ein leerer String. Mit `??` führte das zu `new URL("")` in `app/[locale]/layout.tsx` und einem Build-Absturz; siehe DEPLOYMENT.md für den vollen Fund.
- **Docker-Images enthalten keine Secrets** (verifiziert, siehe DEPLOYMENT.md „Docker-Sicherheit"): `.dockerignore` schließt `.env`/`.env.production`/`.env.*.local` aus; nur `NEXT_PUBLIC_*`-Variablen (ohnehin client-seitig sichtbar) werden als Docker-Build-Args verwendet, echte Secrets ausschließlich zur Laufzeit über `env_file` injiziert.
